...
### 5. Alternatives are real, not strawmen; each one is a self-contained directive
...
**Alternatives are dual-purpose, but each version of the field is single-purpose.** At curation time the alternatives list is architect-facing context: it shows the human (or AI agent) picking the rule what other defensible positions exist, so the selection decision is informed. At emit time, if the architect picks one of them, that alternative's text is emitted to AGENTS.md or CONVENTIONS.md *in place of* the summary; the consumer agent then sees the alternative as the rule's directive, and the summary is not emitted at all.

This means each alternative must be authored to satisfy both audiences with **a single self-contained directive** — the same standard as the summary, per rule #4. No parenthetical tradeoffs, no "(wins on X; loses on Y)" annotations, no meta-commentary on why the alternative might be wrong. When an architect selects an alternative, that text becomes the consumer agent's instruction; parentheticals reading like architect notes would leak into the directive and confuse the agent.

**Where the tradeoff rationale lives.** The reasoning that explains why the rule's default is preferred over each alternative, including the specific drawback of each alternative, belongs in the `why` field. `why` is architect-only and never emitted, so it is the correct home for "this default beats that alternative because Z" reasoning. A well-written `why` carries the comparative reasoning that the parenthetical-in-alternatives form used to carry, and does so in one connected paragraph rather than scattered across the alternatives list.

**Practical test:** if an alternative were promoted to the emitted directive today, would a consumer agent reading it know exactly what to do without parsing any parenthetical context? If not, the alternative is not yet self-contained. Strip the parenthetical and either fold its content into `why` or, if it carries no information the rule's `why` doesn't already cover, drop it.
...
The test: would a reader in a different industry on a different stack recognize the categories as relevant to their work? If yes, the taxonomy is fine and may belong in the summary as calibration. If no, the categories are project-specific and belong out of the rule entirely (or, if architectural-pattern reasoning rather than operational detail, in the `why`).
...
The full diagnostic: read the summary and alternatives out loud as if reading them to a team in a different industry, with a different review cadence, on a different stack. If a phrase only makes sense because the reader shares the author's project context, the phrase is a context leak and belongs in `why` or out of the rule entirely. If a phrase reads as a generic category that any team would recognize, it is calibration, not a leak.
...
**Alternatives have no stable IDs.** The `alternatives` array on each principle is a list of free-text strings today. There is no way to refer to a specific alternative by ID in a commit message, a PR description, or a profile JSON. This is intentional for v0.1: the consumer never reads the non-selected alternatives, and the consumer of a stable alternative ID does not yet exist. v0.2 will likely add IDs, in a format chosen alongside the reverse-engineering import feature so the two designs reinforce each other. **Until that lands, do not invent your own ID conventions in the alternatives array** (no `"ALT-1: text..."` prefixes, no inline tables with custom `id` fields). New rules should keep the alternatives array as plain strings exactly as the existing rules do.

**Alternatives have no per-alternative `why` field.** In v0.1 the comparative reasoning — why the rule's default is preferred over each alternative — lives in the rule-level `why` field as one connected paragraph (per rule #5). This works but mixes the rule's own justification with the rejection rationale for each alternative, and as the number of alternatives grows the `why` field's burden grows with it. v0.2 will likely promote each alternative from a bare string to a small inline table with at least `text` and `why` fields, so the rule-level `why` can return to carrying only the rule's own justification while each alternative carries its own per-alternative rationale alongside its directive. The schema change is non-trivial (touches the principle schema, the lint checks, the emit logic for selected alternatives, and the GUI's review pane), so it is deliberately deferred from v0.1 rather than rushed in. **Until that lands, the rule-level `why` is where per-alternative drawback rationale lives**, and the alternatives array stays as plain strings (no inline tables, no per-alternative metadata).
